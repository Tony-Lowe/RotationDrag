...
def gen_img(
    ...
    # initialize model
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
    model = DragPipeline.from_pretrained(model_path, torch_dtype=torch.float16).to(
        device
    )
    ...
    init_noise = torch.randn(
        [1, 4, height // 8, width // 8], device=device, dtype=model.vae.dtype
    )
    gen_image, intermediate_latents = model(
        prompt=prompt,
    ...
    if height < width:
        ...
        return (
            gr.Image.update(
                value=gen_image, height=int(length * height / width), width=length
            ),
        ...
        return (
            gr.Image.update(value=gen_image, height=length, width=length),
            gr.Image.update(value=None, height=length, width=length),
            gr.Image.update(value=None, height=length, width=length),
            None,
            intermediate_latents,
...
def run_drag_gen(
    ...
    t = model.scheduler.timesteps[args.n_inference_step - args.n_actual_inference_step]
    init_code = deepcopy(
        intermediate_latents_gen[args.n_inference_step - args.n_actual_inference_step]
    )
    init_code_orig = deepcopy(init_code)
    ...
    updated_init_code = drag_diffusion_update_gen(
        model, init_code, t, handle_points, target_points, mask, args
    )
