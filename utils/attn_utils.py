...
class MutualSelfAttentionControl(AttentionBase):
    def __init__(
        ...
        self.layer_idx = (
            layer_idx if layer_idx is not None else list(range(start_layer, 16))
        )
        self.step_idx = (
    ...
    def forward(self, q, k, v, is_cross, place_in_unet, num_heads, **kwargs):
        ...
        if (
            ...
            return super().forward(
                q, k, v, is_cross, place_in_unet, num_heads, **kwargs
            )
        ...
        if self.guidance_scale > 1.0:
            ...
            out_u = F.scaled_dot_product_attention(
                qu, ku[0:1], vu[0:1], attn_mask=None, dropout_p=0.0, is_causal=False
            )
            out_u = torch.cat(
                out_u.chunk(2, dim=2), dim=0
            )  # split the queries into source and target batch
            out_u = rearrange(out_u, "b h n d -> b n (h d)")

            out_c = F.scaled_dot_product_attention(
                qc, kc[0:1], vc[0:1], attn_mask=None, dropout_p=0.0, is_causal=False
            )
            out_c = torch.cat(
                out_c.chunk(2, dim=2), dim=0
            )  # split the queries into source and target batch
            ...
            out = F.scaled_dot_product_attention(
                q, k[0:1], v[0:1], attn_mask=None, dropout_p=0.0, is_causal=False
            )
            out = torch.cat(
                out.chunk(2, dim=2), dim=0
            )  # split the queries into source and target batch
            out = rearrange(out, "b h n d -> b n (h d)")
        return out
...
def override_lora_attn_proc_forward(attn, editor, place_in_unet):
    def forward(
        hidden_states, encoder_hidden_states=None, attention_mask=None, lora_scale=1.0
    ):
        ...
        query, key, value = map(
            lambda t: rearrange(t, "b n (h d) -> b h n d", h=attn.heads),
            (query, key, value),
        )

        # the only difference
        hidden_states = editor(
            query, key, value, is_cross, place_in_unet, attn.heads, scale=attn.scale
        )
...
def register_attention_editor_diffusers(
    model, editor: AttentionBase, attn_processor="attn_proc"
):
    """
        ...
        for name, subnet in net.named_children():
            if net.__class__.__name__ == "Attention":  # spatial Transformer layer
                if attn_processor == "attn_proc":
                    ...
                    net.forward = override_lora_attn_proc_forward(
                        net, editor, place_in_unet
                    )
                else:
...
def register_attention_editor_diffusers_ori(
    model, editor: AttentionBase, attn_processor="attn_proc"
):
    """
    Register a attention editor to Diffuser Pipeline, refer from [Prompt-to-Prompt]
    ...
    def register_editor_ori(net, count, place_in_unet):
        ...
        for name, subnet in net.named_children():
            if net.__class__.__name__ == "Attention":  # spatial Transformer layer
                ...
                count, child_forward_ori = register_editor_ori(
                    subnet, count, place_in_unet
                )
...
def unregister_attention_editor_diffusers(
    ...
    def unregister_editor(net, count, place_in_unet, ori_forward_list):
        ...
        for name, subnet in net.named_children():
            if net.__class__.__name__ == "Attention":  # spatial Transformer layer
                ...
                count = unregister_editor(
                    subnet, count, place_in_unet, ori_forward_list[idx]
                )
            idx += 1
        return count
