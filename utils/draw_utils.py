...
def draw_handle_target_points_r(
    img: PIL.Image.Image,
    handle_points: List,
    target_points: List,
    axis: List,
    ...
    draw = PIL.ImageDraw.Draw(img)
    for handle_point, target_point, ax in zip(handle_points, target_points, axis):
        handle_point = [handle_point[1], handle_point[0]]
        # Draw the handle point
        handle_coords = get_ellipse_coords(handle_point, radius)
        draw.ellipse(handle_coords, fill=color)
        ...
        ax = [ax[1], ax[0]]
        ax_coords = get_ellipse_coords(ax, radius)
        draw.ellipse(ax_coords, fill=color)
